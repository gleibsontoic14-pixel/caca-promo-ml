...
const oldBlock = `async function discoverDomain(query) {
...
function domainCandidateText(c) {
  const attrs = (c.attributes || []).map(a => \\`${'${'}a.value_name || ''} ${'${'}a.name || ''}\\`).join(' ');
  return normalizedText(\\`${'${'}c.category_name || ''} ${'${'}c.domain_id || ''} ${'${'}attrs}\\`);
}
